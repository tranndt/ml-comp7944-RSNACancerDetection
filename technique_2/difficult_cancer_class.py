...
def main(dataset:str, model_name:str, epochs:int, learning_rate:float, batch_size:int, max_lr:float, momentum:float, output_prefix:str, cosine):
    ...
    trainloader, testloader, bias = get_dataset(batch_size, individual=True, get_cancer=True, split_path='../data_splits/hard/')    
...
if __name__ == "__main__":
    ...
    momentums = [0.9, 0.1]
    cosines = [False]
    result_file = "results_difficult_vs_cancer" + str(time.time()) + ".txt"
    results = []
