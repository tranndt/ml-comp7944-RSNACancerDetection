...
def get_dataset(batch_size, individual=False, get_cancer=True):
    ...
    train_dataset = MammogramDataset(os.path.abspath('../data_splits/hard/train_split.csv'), os.path.abspath('../processed_data/'), transform=transform, individual=individual, get_cancer=get_cancer)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=2)
    test_dataset = MammogramDataset(os.path.abspath('../data_splits/hard/test_split.csv'), os.path.abspath('../processed_data/'), transform=transform, individual=individual, get_cancer=get_cancer)
...
def main(dataset:str, model_name:str, epochs:int, learning_rate:float, batch_size:int, max_lr:float, momentum:float, output_prefix:str):
    ...
    best_name, best_accuracy = fit_model(model, trainloader, testloader, device, epochs, learning_rate, max_lr, momentum, "trained_models_difficult_cancer/" + model_name + "/" + output_prefix + dataset + "_" + model_name)
