...
def main(dataset:str, model_name:str, epochs:int, learning_rate:float, batch_size:int, max_lr:float, momentum:float, output_prefix:str):
    ...
    trainloader, testloader = get_dataset(batch_size, individual=True)    
    model = get_model(model_name)
    model.to(device)
    os.makedirs("trained_models/" + model_name +"/", exist_ok=True)
    best_name, best_accuracy = fit_model(model, trainloader, testloader, device, epochs, learning_rate, max_lr, momentum, "trained_models/" + model_name + "/" + output_prefix + dataset + "_" + model_name)
